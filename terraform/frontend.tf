...
resource "null_resource" "getNginxIp" {
  ...
  provisioner "local-exec" {
    command = "docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' mc_nginx > ${var.hostRootPath}/nginx_ip && truncate -s -1 && sudo chmod 777 ${var.hostRootPath}/nginx_ip"
  }
}
